...
int emit_iteration_stmt(Compiler *c, int id, Buf *b, int indent) {
  ...
  int recv = nt_ref(nt, id, "receiver");
  if (!name) return 0;
  /* CRuby checks arity at dispatch, before the iteration starts: a loop
     emitted here never reaches emit_call, so the guard runs here too
     (3.step(4, 1, 2) { } ran the loop with the extra argument dropped). */
  { Buf gb; memset(&gb, 0, sizeof gb);
    if (emit_builtin_arity_guard(c, id, &gb)) {
      buf_printf(b, "%*s%s;\n", indent, "", gb.p ? gb.p : "");
      free(gb.p);
      return 1;
    }
