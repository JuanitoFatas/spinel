...
| A yielding method through a poly receiver, when the yield is nested in a block of its own method, or when the poly call sites pass blocks of differing result type | raises NoMethodError | blocks are fused, not passed: a yielding method is inlined at each call site with the block spliced in, so it has no symbol a dispatch could call. It now also gets a proc-taking form for the poly path, but that is ONE function serving every site, and the code around each yield was typed from the inlined view. Where the sites disagree about the block's result type, or the yield sits inside a nested block whose consumers cannot take the boxed value, no single body serves them -- so those keep raising rather than answer wrongly. Relaxing it means typing the proc form's body independently of the inlined one |
