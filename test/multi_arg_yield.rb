...
# function pointer (`void (*_block)(sp_int, void*)`), so any
...
# function-pointer signature gets N `sp_int` slots) and through
...
#    sp_int 0 both produce the same numeric value.
def mixed_yield
  yield 100, 999
  yield 200
end
...
#    and Spinel's `sp_int 0` produce the same numeric sum.
def varied
  yield 1, 2, 3
  yield 10, 20
  yield 100
