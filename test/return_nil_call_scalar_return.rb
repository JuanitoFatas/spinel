# A method whose return type is a concrete scalar (here sp_int, pinned by the
...
# (`sp_box_nil()`), and returning it raw from an sp_int function is
# uncompilable C ("returning sp_RbVal from a function with incompatible result
# type sp_int"). The fix coerces the box at the return slot
...
class Base
  def self.count(flag)
    if flag
      ...
      AR.adapter.count("x")   # nil receiver -> poly nil-box into the sp_int slot
    end
  end
end
